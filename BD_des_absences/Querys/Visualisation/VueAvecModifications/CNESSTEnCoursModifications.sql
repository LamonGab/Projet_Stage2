CREATE VIEW CNESSTEnCoursModifications AS
...
WHERE cra.DateDébutCSQ <= GETDATE() AND (cra.DateFinCSQ IS NULL OR cra.DateFinCSQ >= GETDATE()) AND (cra.Conséquence IS NOT NULL OR cra.DateDébutCSQ IS NOT NULL OR
cra.DateFinCSQ IS NOT NULL OR ca.DateRetourPrévue IS NOT NULL)