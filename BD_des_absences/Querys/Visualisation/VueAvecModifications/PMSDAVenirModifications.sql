CREATE VIEW PMSDAVenirModifications AS
SELECT DISTINCT cra.Matricule, cra.Conséquence, cra.DateDébutCSQ, cra.DateFinCSQ, cpp.CodeDePaie, cd.Description, p.NuméroÉvénement, p.DateHeureModification, p.StatutDonnée
...
WHERE cra.DateDébutCSQ > GETDATE() AND (cra.Matricule IS NOT NULL OR cra.Conséquence IS NOT NULL OR cra.DateDébutCSQ IS NOT NULL OR cra.DateFinCSQ IS NOT NULL)